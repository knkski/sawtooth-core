...
class ClientException(ClickException):
    pass
...
class BattleshipClient:
    ...
    def __init__(self, base_url, keyfile, wait=None):
        ...
        try:
            ...
            raise IOError(f"Failed to read keys: {err}.")
    ...
    def list_games(self, auth_user=None, auth_password=None):
        ...
        return [
            json.loads(b64decode(entry["data"]).decode())
            for entry in yaml.safe_load(result)["data"]
        ]
    ...
    def _get_status(self, batch_id, wait):
        try:
            response = self._send_request('batch_statuses?id={}&wait={}'.format(batch_id, wait))
            result = yaml.safe_load(response)

            if result['data'][0]['invalid_transactions']:
                raise ClientException(result['data'][0]['invalid_transactions'][0]['message'])
    ...
    def _send_request(self, suffix, data=None, content_type=None, name=None, auth_user=None, auth_password=None):
        ...
        if auth_user is not None:
            auth_string = f"{auth_user}:{auth_password}"
        ...
        try:
            ...
            if result.status_code == 404:
                raise ClientException(f"No such game: {name}")

            elif not result.ok:
                raise ClientException(f"Error {result.status_code}: {result.reason}")
