...
use messages::batch::{Batch, BatchHeader, BatchList};
use messages::transaction::{Transaction, TransactionHeader};
use protobuf::{Message, ProtobufError, RepeatedField};
use signing::{Error as SigningError, Signer};
use std::fmt;
...
impl From<SystemTimeError> for Error {
    fn from(err: SystemTimeError) -> Self {
        Error {
            inner: Context::new(ErrorKind::TimeError(format!("{}", err))),
        }
    }
}
...
impl<'a> TransactionBuilder<'a> {
    ...
    pub fn build(self) -> Result<(Transaction, TransactionHeader), Error> {
        ...
        let default_nonce = SystemTime::now().duration_since(UNIX_EPOCH)?.as_secs();

        txn_header.set_family_name(family_name);
        txn_header.set_family_version(family_version);
...
#[cfg(test)]
mod tests {
    use super::super::signing::secp256k1::Secp256k1PrivateKey;
    use super::super::signing::{create_context, Signer};
